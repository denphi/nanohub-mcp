...
def _runner_environment(app_path):
    # type: (str) -> dict
    """Build the environment used by a runner subprocess.

    The child gets the app directory and whatever the session already had,
    and nothing else.  In particular it does NOT get the launcher's own
    package root: a pip-installed launcher lives in a shared site-packages,
    and putting that on the child's path hands it every library installed
    beside the framework — which is how a launcher's NumPy ends up in an
    interpreter it was not built for.

    The version guarantee that motivated it is delivered by the runner
    instead:
    ``_pin_framework`` imports the launcher's ``nanohubmcp`` from its own
    file, so the child cannot pick up a different framework version without
    anything else coming along for the ride.
    """
    env = os.environ.copy()
    app_dir = os.path.dirname(os.path.abspath(app_path))
