...
def _runner_environment(app_path):
    ...
    ``--python-env`` deliberately changes interpreters so an app can use
    environment-specific scientific packages.  That interpreter may also have
    an older ``nanohubmcp`` installed, however.  A runner generated by one
